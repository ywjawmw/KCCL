...
herb_pair_prompt = "你是一个资深的中药信息分析专家，<input_info>要求总结的语言流畅，逻辑通顺。用一段话进行描述，不要分段，不能丢失输入的任何关系信息。"

sym_pair_prompt = "你是一个资深的症状信息分析专家，<input_info>要求总结的语言流畅，逻辑通顺。用一段话进行描述，不要分段，不能丢失输入的任何关系信息。"
